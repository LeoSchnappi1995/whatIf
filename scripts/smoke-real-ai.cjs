...
async function main() {
  ...
  const input = {
    ...
    story: { title: '2056年的我们', setting: '2056年的上海，AI拥有独立身份，旧城区仍保留纸质书店。', worldview: { name: '未来平行线', stylePrompt: 'grounded cinematic animation, near-future Shanghai' } },
    characters: [
  ...
  const world = await ai.generateWorldviewImage({ name: '2056年的上海旧城区', description: '原创电影动画风格，雨夜旧书店、悬浮公共交通与纸质书共存，无人物。' });
  const compiled = await ai.compileSeedance({ ...input, directorPlan: director });
  const video = await ai.createVideo({ prompt: compiled.prompt, copyrightSafePrompt: `${compiled.prompt}\nAll people are original fictional adults in cinematic animation.` });
  ...
  process.stdout.write(JSON.stringify({
    config,
    director: { title: director.title, capacity: director.capacity, shotCount: director.shots.length, promptVersion: director.promptVersion },
    image: { ok: Boolean(world.imageUrl), promptVersion: world.promptVersion },
    video: { taskId: video.providerTaskId, inputMode: video.inputMode, status: status.status, hasVideo: Boolean(status.videoUrl), error: status.error || '', promptVersion: compiled.promptVersion },
  }, null, 2));
}
